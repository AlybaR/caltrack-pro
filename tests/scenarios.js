...
const SCENARIOS = [
    /* ─── ONBOARDING ─────────────────────────────────────────── */
    {
        ...
        async run(ctx) {
            ...
            // Smart goal suggest should appear
            await ctx.wait(200);
            ...
            // ETA preview should be present after step 3 mounts
            await ctx.wait(300);
    ...
    /* ─── DASHBOARD ──────────────────────────────────────────── */
    {
        ...
        target: '< 3 sec, 0 tap',
        async run(ctx) {
            ctx.setStorage('settings', SAMPLE_SETTINGS);
            await ctx.wait(100);
            await ctx.waitFor('#page-dash');
            await ctx.click('#nb-dash').catch(() => {}); // ensure on dash
            await ctx.waitFor('#dash-greeting', 5000);
    ...
    /* ─── JOURNAL — quick add ────────────────────────────────── */
    {
        ...
        async run(ctx) {
            ctx.setStorage('settings', SAMPLE_SETTINGS);
            // Pré-remplir un aliment récent
            ctx.setStorage('recent_foods', [
            ...
            await ctx.wait(200);
            // Trouver un bouton de récent (qbtn dans la première section ouverte)
...

    {
        ...
        async run(ctx) {
            ...
            await ctx.waitFor('#meals-container');
            await ctx.wait(200);
            // Switcher sur l'onglet Manuel du 1er repas ouvert
    ...
    /* ─── SPORT — cardio ─────────────────────────────────────── */
    {
        ...
        async run(ctx) {
            ...
            await ctx.wait(200);
            // Cliquer sur le 1er preset cardio (depuis Récents si vide → fallback Cardio)
            // On va à la catégorie 🏃 Cardio
            const cardioPill = ctx.$$('.ex-cat-btn').find(b => b.textContent.includes('Cardio'));
            if (cardioPill) { cardioPill.click(); ctx.tapCount++; await ctx.wait(150); }
            ...
            await ctx.wait(80);
            // Valider
            const validBtn = ctx.$$('button').find(b => b.textContent.includes('Ajouter') || b.textContent.includes('Valider'));
            if (validBtn) { validBtn.click(); ctx.tapCount++; }
            else if (typeof ctx.$.confirmCardio === 'function') ctx.$.confirmCardio();
            await ctx.wait(400);
            const dk = new Date().toISOString().slice(0, 10);
            const day = ctx.getStorage('day_' + dk);
    ...
    /* ─── OBJECTIF ───────────────────────────────────────────── */
    {
        ...
        async run(ctx) {
            ...
            ctx.setStorage('weight-history', wh);
            await ctx.wait(100);
            await ctx.click('#nb-dash');
